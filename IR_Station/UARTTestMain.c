...
//data array 
//int data[6] = {start_hi, start_lo, one_hi, one_lo, zero_hi, zero_lo};
int data[6] = {10, 5, 11, 4, 4, 4};
...
void PortC_Init(){ unsigned long delay;
	...
	GPIO_PORTB_AMSEL_R&= 0x0F;					//disable analog for PC7,6,5,4
...
void SysTick_Handler(void){
 ...
 void GPIOPortF_Handler(void){
	 ...
	 if(GPIO_PORTF_DATA_R & 0x10){
		 LED_off();
		 //GPIO_PORTC_DATA_R = GPIO_PORTC_DATA_R ^ 0x80;	
